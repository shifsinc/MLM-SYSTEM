...
<div class="modal-body">
	...
	<form role="form" id="form_edit" action="soap_func.php?type=shopedititem&tb=user" method="post" enctype="multipart/form-data">
		...
		<!--div class="form-group"><label>State</label><span class="text-danger"><strong> (Please select at least one state)</strong></span>
									<div class="row ">
									    <?php
											...
											$result = $db->where('*',$tb,'mv_user_state.mv_user_id',$uid);
										    $tb1 = 'mv_state JOIN mv_item_state ON mv_state.mv_state_id=mv_item_state.mv_state_id ';
											$result2 = $db->where('*',$tb1,'mv_item_state.mv_item_id',$id);
									   
												foreach($result as $row){
												    		
												    ?>
								            <div class="i-checks col-md-3 "><input type="checkbox" name="statechecked[]" value="<?php echo $row['mv_state_id']; ?>" <?php if ($result2['mv_item_id'] == $id): ?>
					checked="checked"
				<?php endif ?> /> <?php echo $row['mv_state_name']; ?></div>
								            <?php }?>
								        </div>
									</div-->
